...
contract MockTransactionInclusionVerifier is ITransactionInclusionVerifier {
    PublicValuesStruct internal mockReturn = PublicValuesStruct({
        ...
        committedTransactionHash: bytes32(uint256(0x456)),
...
contract TxInclusionPreciseSlasherTest is Test {
    ...
    function testRevert_Slash_TransactionWasIncluded() public {
        ...
        mockVerifier.setMockReturn(
            _makeProofOutput(COMMITTED_BLOCK_NUMBER, COMMITTED_TRANSACTION_HASH, COMMITTED_TRANSACTION_INDEX, true)
        );
    ...
    function testRevert_Slash_MissingCanonicalBlockHash() public {
        ...
        vm.prank(user);
        vm.expectRevert(TxInclusionPreciseSlasher.MissingCanonicalBlockHash.selector);
        slasher.slash(commitment, proposer, v, r, s, _encodeProofOutput(true), _dummyProof());
    }
    ...
    function testRevert_Slash_BlockHashMismatch() public {
        ...
        vm.prank(user);
        vm.expectRevert(TxInclusionPreciseSlasher.BlockHashMismatch.selector);
        slasher.slash(commitment, proposer, v, r, s, _encodeProofOutput(true), _dummyProof());
    }

    function testRevert_Slash_CommittedTransactionHashMismatch() public {
        _bondProposer(1 ether);
        _registerCanonicalBlock();
        ...
        mockVerifier.setMockReturn(
            _makeProofOutputWithCommittedHash(
        ...
        vm.prank(user);
        vm.expectRevert(TxInclusionPreciseSlasher.CommittedTransactionHashMismatch.selector);
        slasher.slash(commitment, proposer, v, r, s, _encodeProofOutput(true), _dummyProof());
    }

    function testRevert_Slash_TransactionCannotBeIncluded() public {
        _bondProposer(1 ether);
        _registerCanonicalBlock();
        ...
        vm.prank(user);
        vm.expectRevert(TxInclusionPreciseSlasher.TransactionCannotBeIncluded.selector);
        slasher.slash(commitment, proposer, v, r, s, _encodeProofOutput(true), _dummyProof());
    }
    ...
    function testRevert_Slash_InvalidNoTransactionProof() public {
        ...
        mockVerifier.setMockReturn(
            _makeProofOutput(COMMITTED_BLOCK_NUMBER, INCLUDED_TRANSACTION_HASH, COMMITTED_TRANSACTION_INDEX, false)
        );
    ...
    function testRevert_Slash_TransactionIndexMismatch() public {
        ...
        mockVerifier.setMockReturn(
            _makeProofOutput(COMMITTED_BLOCK_NUMBER, INCLUDED_TRANSACTION_HASH, COMMITTED_TRANSACTION_INDEX + 2, true)
        );
    ...
    function testRevert_Slash_BlockNumberMismatch() public {
        ...
        mockVerifier.setMockReturn(
            _makeProofOutput(COMMITTED_BLOCK_NUMBER + 1, INCLUDED_TRANSACTION_HASH, COMMITTED_TRANSACTION_INDEX, true)
        );
        ...
        returns (PublicValuesStruct memory)
    {
        return PublicValuesStruct({
            ...
            committedTransactionHash: COMMITTED_TRANSACTION_HASH,
    ...
    function _makeProofOutputWithCommittedHash(
    ...
    ) internal pure returns (PublicValuesStruct memory) {
        return PublicValuesStruct({
            blockHash: PROOF_BLOCK_HASH,
            blockNumber: blockNumber,
            committedTransactionHash: committedTransactionHash,
            transactionHash: transactionHash,
            transactionIndex: transactionIndex,
            isIncluded: isIncluded,
            transactionCanBeIncluded: transactionCanBeIncluded,
            verifiedAgainstRoot: PROOF_ROOT
        });
    }
